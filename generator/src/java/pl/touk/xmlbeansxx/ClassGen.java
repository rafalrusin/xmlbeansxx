	...
	private void genInitSchemaType(SchemaType st) {
...

		{
			...
			if (st.getName() == null) {
				...
				out.cpp.println("  st.xsdNamespaceID="+nsLinks.getVarName(st.getName().getNamespaceURI())+";");
			}
			...
			}
			{
				...
				for (int i = 0; i < properties.length; i++) {
					...
					out.cpp.println("  st.elements[\"" + p.getName().getLocalPart()
							+ "\"]=xmlbeansxx::SchemaPropertyPtr(new xmlbeansxx::SchemaProperty(" + (i + 1) + ","+genCreateFn(p.getType())+"()->getSchemaType(), " + genDefaultSingletonStringPtr(p) + "));");
				}
			}
			{
